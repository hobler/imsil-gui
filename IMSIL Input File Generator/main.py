...
def read_existing_input_file(file_path):
    """
        read_existing_input_file will use the path file_path to read the
...
def center_window(tk_window):
    """
        center_window place the given Tk to the center of the screen
...
class ImsilInputFileGenerator:

    def get_all_parameter_values(self, parent):
    ...
    def set_parameter_value(self, notebook_widget, tab_name, parameter_name,
                            parameter_value):
        """
    ...
    def __init__(self, input_file_path, type_of_simulation):
        root = tk.Tk()
        root.title('IMSIL Input File Generator')
...
class WelcomeWindow(tk.Tk):
    """
        ...
        existing IMSIL input file and a button to open the IMSIL input file
        generator.
        ...
        These parameters, so called "basic" paramters, that are for this
    ...
    def __init__(self):
        ...
        self.header_label = tk.Label(
            self.frame, width=self.WINDOW_WIDTH,
        ...
        self.welcome_message_label = tk.Label(
            self.frame, width=self.WINDOW_WIDTH, height=10, anchor=tk.NW,
            text="Welcome to the input file generator for IMSIL.\n\n"
                 "With this tool you can generate an input file for "
                 "IMSIL.\n\n" +
                 "You can also edit an existing input file by loading it in "
                 "this window below.",
            justify=tk.LEFT,
        ).grid(row=1, column=0, columnspan=3, padx=10, sticky="NESW")
        ...
        self.choose_existing_file_label = tk.Label(
            ...
            .grid(row=3, column=0, pady=5, sticky="NESW")
        ...
        # Add button to open the imsil input file generator
        self.btn_continue = tk.Button(
            self.frame, text='Open IMSIL input file generator', padx=10,
            command=self.open_imsil_input_file_generator, pady=10
    ...
    def load_file(self):
        """
            load_file open a dialog to pick up a file. After the user select a
    ...
    def open_imsil_input_file_generator(self):
        """
            open_imsil_input_file_generator will close this window and open
