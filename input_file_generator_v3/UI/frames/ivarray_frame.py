...
class IndexVariableArrayFrame(BlancFrame):
    ...
    def set_values(self, values, nr_pre, natom_pre):
        ...
        :param natom_pre: previous number of atoms
        """
        ...
        if "POINT" in values[2][1]:
            ...
            for child in self.children:
                if "entry" in child:
                    ...
                    if entry_nr < len(values[0]):
                        self.set_entry(entry, values[0][entry_nr])
                    else:
        ...
        # standard case
            ...
            if dim == 1:
                if ("ATOM1" in index_var_list or "ATOM2" in index_var_list or
                    ...
                    width_pre = 1
                      ...
                      "POINTS" in str(values[2][0])):
                    # NR x 1 array (+1 because of region0)
                    width_now = self.n_r + 1
                    width_pre = nr_pre + 1
                elif "REGION" in index_var_list:
                    # 1 x NR array
                    width_now = 1
            ...
            # value_nr is the position in the entry value list (values[0])
            entry_nr = 0
            grid_entry_nr = -1
            value_nr = 0
            for child in self.children:
                if "entry" in child:
                    ...
                    if grid_entry_nr >= 0:
                        # repeat is there for when the new width is smaller
                        # than the old one
                        # if that is the case, we have to increment value_nr
                        # a few times to make the positions match again
                        repeat = True
                        while repeat:
                            # get the 2D coordinates of where
                            # this value was in the previous grid
                            val_pos = [value_nr // width_pre,
                                       value_nr % width_pre]
                            # get the 2D coordinates of the current entry field
                            entry_pos = [grid_entry_nr // width_now,
                                         grid_entry_nr % width_now]

                            # if there are values left in the list
                            ...
                            if value_nr + 1 < len(values[0])\
                                ...
                                self.set_entry(entry, values[0][value_nr + 1])
                                value_nr += 1
                                # do not repeat, because right value was found
                                repeat = False
                            # if width_now < width_pre as mentioned before
                            # and if there are values left
                            elif width_now < width_pre \
                                    and value_nr + 1 < len(values[0]):
                                # try same entry with the next value
                                value_nr += 1
                                repeat = True
                            # no entries left in the list
                            else:
                                self.set_entry(entry, "")
                                repeat = False
                    else:
